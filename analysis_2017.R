#because of using the pre-computed data queries cannot/ should not be run because that would require using a dataset and the sql file
#thus pre-computed files are used and even merged when necessary
#some of the steps are cleaning data and show how to reach the form of pre-computed files and they usually add further columns as well
#because of not running the sql file the form of date has to be changed from factor to date to be able to run the visualizations



trips<-read.csv("daily_trips_by_geography.csv")
...
#write_csv(trips, "daily_trips_by_geography.csv")


######
###### this part is creating the columns and filling them based on a database --> cleaning process
######
...
#setting the fillings and levels for various columns and setting when we start having data on various types of data
#also defnes that Uber should not have data between the period of 2014-10-01 - 2014-12-31 -- still do not know why, as the Uber timeline does not tell anything why it wouldnt have data during that time 
trips = trips %>%
  group_by(car_type) %>%
  complete(
    date = date_seq,
...
######
###### this part is estimating monthly pick ups for Juno and Gett as there is no data on that for those
###### and then saying whether the number is an estimate or not in a separate column to be able to tell which ones were imputed --> cleaning process
...
#add the previously calculated monthly an daily fractions to the missing values of Gett and Juno
trips = trips %>%
...
trips$date<-as.Date(trips$date) #added line by Nicol

#get the total trips of each type of car on a daily basis
# citywide total
...
#get the total number of trips by car type for the whole observed period
totals_by_car_type_label_data = totals_by_car_type %>%
  filter(date == max(date)) %>%
  mutate(yval = case_when(
    car_type == "Juno" ~ monthly+ 400e3,
    car_type == "Green taxis" ~ monthly - 500e3,      #dont really get the numbers or the choice of these specific cars because y value becomes an added column and only the numbers for those 2 cars change and otherwise the other cars get the monthly number as yval (Nicol)
    TRUE ~ monthly
  ))

#creating ggplot object of monthly taxi pickups -- group enables to show different car type data as separate line 
...
#getting the total pick up number for the two main groups with excluding missing values, using mutate function to create a new variable Ridehailing_2014View()
...
#sum all the cases by parent label -- not really sure why they chose the exact date or why they need a differnt yval than the monthly value
...
#creating a ggplot object which can be plotted latter
...
#creating data frame for Manhattan pick ups and redoing everything done previously to the whole dataset just now examining Manhattan
# Manhattan
...
#despite now adding yval as a new variable with changing the same categories as previously, now other numbers are used
...
#again repeating everything but with now looking at central Manhattan
...
#again, same type of car but different number added in case of yval 
...
#doing the same as previously with different location 
...
#weirdly no added or subtracted number to yval but there is also an extra column with saying that none of the types were estimated/imputed from those categories
...
#doing the same as previously but with examining airports 
# JFK + LGA airports
...
#saving every ggplot object as a png into the folder
png("totals_by_car_type.png", width = 800, height = 800)
print(totals_by_car_type_plot)
dev.off()

png("totals_by_parent_type.png", width = 800, height = 800)
print(totals_by_parent_type_plot)
dev.off()

png("manhattan_by_car_type.png", width = 800, height = 800)
print(manhattan_by_car_type_plot)
dev.off()

png("manhattan_by_parent_type.png", width = 800, height = 800)
print(manhattan_by_parent_type_plot)
dev.off()

png("manhattan_cbd_by_car_type.png", width = 800, height = 800)
print(manhattan_cbd_by_car_type_plot)
dev.off()

png("manhattan_cbd_by_parent_type.png", width = 800, height = 800)
print(manhattan_cbd_by_parent_type_plot)
dev.off()

png("outer_boroughs_by_car_type.png", width = 800, height = 800)
print(outer_boroughs_by_car_type_plot)
dev.off()

png("outer_boroughs_by_parent_type.png", width = 800, height = 800)
print(outer_boroughs_by_parent_type_plot)
dev.off()

png("airports_by_car_type.png", width = 800, height = 800)
print(airports_by_car_type_plot)
dev.off()

png("airports_by_parent_type.png", width = 800, height = 800)
print(airports_by_parent_type_plot)
dev.off()


#this exludes green and yellow taxis, creates variable for weekly pick ups with a 
#rolling sum, creates variables for market share with dividing numbers with the 
#sum of all numbers then creating variables for daily, weekly and monthly change with
#subtracting the values of the share 1,2, 28 days before that day
...
#creating a ggplot object for visualizing weekly market share of Uber and Lyft
...
png("ridehail_market_share.png", width = 800, height = 800)
print(ridehail_market_share)
dev.off()

# Uber and Lyft weeks with biggest INCREASE in ride-hailing market share
#choosing top 10 biggest value from each category
ridehail_trips %>%
  filter(
...
#choosing to show bottom 10 values of each category
ridehail_trips %>%
  filter(
    car_type %in% c("Uber", "Lyft"),
    geo == "total",
...
#read in already existing csv files instead of working with sql and a database
jfk_hourly_pickups<-read.csv("jfk_hourly_pickups.csv")
jfk_hourly_pickups$pickup_hour<-as.POSIXct(jfk_hourly_pickups$pickup_hour, format="%Y-%m-%d %H:%M:%S")

#set the time of the beginning of the protest
protest_hour = as.POSIXct("2017-01-28 18:00:00")

#making a dataframe to compare pickups within the hour of the taxi strike at the jfk airport 
...
#creating a ggplot vector to show hourly pick up rate between 26th January 
#and 1st February to see whether the strike affected pick up rate within the hour of the strike
...
png("jfk_hourly_pickups.png", width = 800, height = 800)
print(jfk_hourly)
dev.off()
...
election<-read.csv("election_results_by_taxi_zone.csv")
uber_vs_lyft_carto_data<-read.csv("uber_vs_lyft_carto_data.csv")

#running a regression and then looking at the amount of correlation 
#between the share of switch from uber to lyft and the share of ploitical 
#support for Clinton or Stein in boroughs
...
# merging the two datasets to be able to run the following visualizations 
election<-merge(election_results_by_taxi_zone, uber_vs_lyft_carto_data, by="locationid")

#visualizing how Clinton vote % and Lyft market share increase can be related
clinton = election %>%
...
#saving plots as png into the directory
png("lyft_vs_clinton.png", width = 800, height = 800)
print(clinton)
dev.off()

png("lyft_vs_jill_stein.png", width = 800, height = 800)
print(stein)
dev.off()
